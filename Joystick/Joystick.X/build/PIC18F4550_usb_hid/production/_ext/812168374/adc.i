...
int ADC_multiButton(ADC_CHANNEL channel) {
    ...
    uint16_t adc_volt = ADC_Readbits(channel);
        ...
        if (adc_volt<=1024 && adc_volt>=853){
      return 5;
     }else if (adc_volt<=852 && adc_volt>=682){
      return 4;
  }else if (adc_volt<=681 && adc_volt>=511){
      return 3;
  }else if (adc_volt<=510 && adc_volt>=340){
      return 2;
  }else if (adc_volt<=339 && adc_volt>=169){
      return 1;
        } else {
            return 0;
        }
