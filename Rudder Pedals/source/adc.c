...
uint8_t ADC_HatSwitch(ADC_CHANNEL channel) {
    ...
    uint16_t adc_volt = ADC_Readbits(channel);
    	...
    	if (adc_volt<=4096 && adc_volt>=3072){          //Left
		    return 0x6;
		}else if (adc_volt<=3071 && adc_volt>=2048){    //Down
		    return 0x4;
		}else if (adc_volt<=2047 && adc_volt>=1024){     //Right
		    return 0x2;
		}else if (adc_volt<=1023 && adc_volt>=0){       //Up
		    return 0x0;
        } else {                                      //Null
            return 0x8;
        }
